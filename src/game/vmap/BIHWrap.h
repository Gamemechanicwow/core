...
public:
    ...
    void intersectRay(const Ray& r, RayCallback& intersectCallback, float& maxDist, bool ignoreM2Model, bool stopAtFirstHit = true )
    {
        balance();
        MDLCallback<RayCallback> temp_cb(intersectCallback, m_objects.getCArray(), m_objects.size());
        m_tree.intersectRay(r, temp_cb, maxDist, stopAtFirstHit, ignoreM2Model);
    }
